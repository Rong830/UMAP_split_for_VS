...
props = {0: [1, 'property', 'regression', 4, 14]}
...
FOLD = getConfig("Details", "fold", "1");
SPLIT = getConfig("Details", "split", "umap");
...
# Function to read cluster-based set mappings
def read_set_ids_with_cluster(id_csv_file, split, fold):
    fold = int(fold)
    set_mapping = {}
    if split == 'random':
         split_column = 'Random_Cluster_ID'
    elif split == 'scaffold':
         split_column = 'Scaffold_Cluster_ID'
    elif split == 'butina':
         split_column = 'Butina_Cluster_ID'
    elif split == 'umap':
         split_column = 'Cluster_ID'
    val_fold = 7 if fold == 1 else fold - 1
    
    with open(id_csv_file, "r") as f:
        reader = csv.DictReader(f)
        for row in reader:
            nsc_id = row["NSC"].strip()  # Molecule ID
            cluster_id = int(row[split_column].strip())  # Choose splitting method
            # Example split logic
            if cluster_id == fold:
                set_mapping[nsc_id] = "test"
            elif cluster_id == val_fold:
                set_mapping[nsc_id] = "val"
            else:
                set_mapping[nsc_id] = "train"
    return set_mapping

# Main function to process and split based on IDs
def analyzeDescrFile(fname, id_csv_file, split, fold):
    # Read the set mappings from the ID CSV
    set_mapping = read_set_ids_with_cluster(id_csv_file, split, fold)
   ...
   #  n=0
    for row in csv.reader(open(fname, "r")):
      ...
      #       break
        if first_row:
            ...
            for i, prop in enumerate(row):
                if prop.lower() == "smiles":
                    ind_mol = i  # Index of the SMILES column
                elif prop == "NLOGGI50_N":
                    ind_property = i  # Index of the prediction column
            if ind_mol is None or ind_property is None:
                raise ValueError("Missing SMILES or NLOGGI50_N column in data CSV")
            continue

        # Extract NSC ID and molecule SMILES
        nsc_id = row[1].strip()  # First column is assumed to be NSC ID
        dataset_type = set_mapping.get(nsc_id, None)
        if dataset_type is None:
            continue  # Skip if NSC ID is not in the set mapping
        ...
        # Extract the 'NLOGGI50_N' property value
        prop_value = row[ind_property].strip()
        val = float(prop_value) if prop_value else 0  # Set 0 for missing values
        mask = 1 if prop_value else 0  # Binary mask (1 if present, 0 if missing)

        arr = list(set(arr))  # Remove duplicate SMILES

        for step in range(len(arr)):
            sample = [arr[step], np.array([val], dtype=np.float32), np.array([mask], dtype=np.int8)]
    ...
    findBoundaries(train_DS);
    #  findBoundaries(val_DS);
    #  findBoundaries(test_DS);
...
if __name__ == "__main__":
    ...
    if TRAIN == "True":
        ...
        id_csv_file = '/rds/general/user/qg622/home/Y2/ScaffoldSplitsOverestimateVS/data/clustering_id_k7.csv'
      #   id_csv_file = './data/clustering_id_k7.csv'
        train_DS, val_DS, test_DS = analyzeDescrFile(
           TRAIN_FILE,
           id_csv_file,
           SPLIT,
           FOLD
        ...
        nall = len(train_DS) + len(val_DS) + len(test_DS);
        ...
        if EARLY_STOPPING == 0:
           ...
           print("Trainig samples:", len(train_DS), "validation:", len(val_DS), "test:", len(test_DS));  

           train_generator = data_generator(train_DS);
           valid_generator = data_generator(val_DS);
           test_generator = data_generator(test_DS);
         ...
         #   train_DS
         #    ['c1c2cc(C(=O)Nc3ccc(F)c(F)c3)c(Nc3cc(OC)c(OC)c(OC)c3)nc2ccc1', array([5.471], dtype=float32), array([1], dtype=int8)]
         #   np.shape(DSC_TRAIN[0][0][0]) (32, 79, 64)
           valid_generator = data_generator2(DSC_VALID);
           test_generator = data_generator2(DSC_TEST);
        ...
        if EARLY_STOPPING > 0:
         ...
         #   mdl.save_weights("model.h5");
           ...
           x, y = gen_data(test_DS);
           internal = encoder.predict( [x[0], x[1]]);
           p = [internal];
           for i in range(len(props)):
             p.extend([x[i+2]]);
           y_pred = mdl.predict(p, verbose=1, batch_size=32);

           # Separate test dataset into SMILES, property values, and masks
           smiles_list = [sample[0] for sample in test_DS]  # SMILES strings
           true_values = [sample[1][0] for sample in test_DS]  # True NLOGGI50_N values (can be zeros if missing)
           masks = [sample[2][0] for sample in test_DS]  # Binary masks indicating missing/valid values

           # Create NumPy array of SMILES for prediction
           smiles_input = np.array(smiles_list)

           # Prepare DataFrame for saving
           predictions_df = pd.DataFrame({
               "SMILES": smiles_list,
               "True_NLOGGI50_N": true_values,
               "Predicted_NLOGGI50_N": y_pred.flatten()*10+4,  # Flatten predictions array
               # "Mask": masks  # Indicates whether the original value was available (1) or missing (0)
           })
